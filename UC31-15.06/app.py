...
def personalizar():
    ...
    return render_template(
        ...
        cor_atual=session.get('cor', '#000000')
    )
...
@app.route('/visitas-contador')
def visitas_contador():
...
def adicionar_ao_carrinho(produto_id):
    ...
    carrinho_atual = session.get('carrinho', [])
    item_existente = next((item for item in carrinho_atual if item['id'] == produto_id), None)
...
def remover_do_carrinho(produto_id):
    carrinho_atual = session.get('carrinho', [])
    novo_carrinho = [item for item in carrinho_atual if item['id'] != produto_id]
...
@app.route('/favoritos')
def favoritos():
    ids_favoritos = session.get('favoritos', [])
    lista_favoritos = [p for p in PRODUTOS if p['id'] in ids_favoritos]
    return render_template('favoritos.html', produtos=PRODUTOS, favoritos=lista_favoritos)

@app.route('/favoritos/adicionar/<int:produto_id>')
def adicionar_aos_favoritos(produto_id):
    produto = next((p for p in PRODUTOS if p['id'] == produto_id), None)
    ...
    favoritos_atual = session['favoritos']

    if produto_id not in favoritos_atual:
        favoritos_atual.append(produto_id)
        session['favoritos'] = favoritos_atual
        session.modified = True
...
def remover_dos_favoritos(produto_id):
    favoritos_atual = session.get('favoritos', [])
    novo_favoritos = [id for id in favoritos_atual if id != produto_id]

    session['favoritos'] = novo_favoritos
    session.modified = True
